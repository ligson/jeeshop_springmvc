...
<div class="navbar navbar-default"
	...
	<div class="container" style="min-height: 10px;margin-top:5px;margin-bottom:5px;border: 0px solid red;">
		<div class="row">
			<div class="col-xs-3">
				<a href="<%=SystemManager.systemSetting.getWww()%>"><img style="max-height: 50px;" alt="myshop-logo"
																		 src="<%=SystemManager.systemSetting.getLog()%>"/></a>
			</div>
					...
					<div class="form-group btn-group">
						<div class="input-group">
							<input type="text" name="key" id="key" class="form-control input-sm"
								   style="border: 2px solid red;border-right: 0px;"
					...
					<c:choose>
						<c:when test="${sessionScope.user_info == null}">
						   <span class="col-xs-12" id="loginOrRegSpan" style="font-size: 14px;">
		          			<a href="<%=request.getContextPath()%>/user/login.html">登陆</a>|<a
								   href="<%=request.getContextPath()%>/user/register.html">注册</a>
		          		   </span>
						</c:when>
						<c:otherwise>
							  ...
							  <ul class="dropdown-menu" id="myshopMenu" role="menu"
								  ...
								  <li>
									  <a href="<%=request.getContextPath()%>/user/<%=FrontContainer.user_leftMenu_user%>.html"><span
											  class="glyphicon glyphicon-user"></span>&nbsp;个人资料</a></li>
								  <li>
									  <a href="<%=request.getContextPath()%>/user/<%=FrontContainer.user_leftMenu_topwd%>.html"><span
											  class="glyphicon glyphicon-screenshot"></span>&nbsp;修改密码</a></li>
								  <li class="divider"></li>
								  <li>
									  <a href="<%=request.getContextPath()%>/user/<%=FrontContainer.user_leftMenu_orders%>.html"><span
											  class="glyphicon glyphicon-th"></span>&nbsp;我的订单</a></li>
								  <li>
									  <a href="<%=request.getContextPath()%>/user/<%=FrontContainer.user_leftMenu_address%>.html"><span
											  class="glyphicon glyphicon-send"></span>&nbsp;配送地址</a></li>
								  <li>
									  <a href="<%=request.getContextPath()%>/user/<%=FrontContainer.user_leftMenu_favorite%>.html"><span
											  class="glyphicon glyphicon-tags"></span>&nbsp;收藏夹</a></li>
								  <li class="divider"></li>
								  <li>
									  <a href="<%=request.getContextPath()%>/user/<%=FrontContainer.user_leftMenu_exit%>.html"><span
											  class="glyphicon glyphicon-pause"></span>&nbsp;退出系统</a></li>
							  </ul>
							</span>
			          		</span>
		...
		<div class="navbar-header col-xs-3" style="text-align: center;" id="navbar-header">
			<c:choose>
				<c:when test="${sessionScope.selectMenu == ''}">
					<a class="navbar-brand" href="<%=SystemManager.systemSetting.getWww()%>"
					   style="font-weight: bold;margin-left: 5px;">全部商品分类</a>
				</c:when>
				<c:otherwise>
					<a class="navbar-brand" href="<%=SystemManager.systemSetting.getWww()%>"
					   style="font-weight: bold;margin-left: 5px;">全部商品分类</a>
					...
					<c:if test="${row.showInNav =='y' }">
						<c:choose>
							<c:when test="${row.id == sessionScope.selectMenu}}">
								<li class="active"><a
										href="<%=request.getContextPath()%>/catalog/<c:out  value="${row.code}" />.html">
